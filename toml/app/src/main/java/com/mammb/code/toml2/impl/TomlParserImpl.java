...
import com.mammb.code.toml2.api.TomlParser;
...
public class TomlParserImpl implements TomlParser {
    ...
    private final TomlContext tomlContext;

    public TomlParserImpl(InputStream in, TomlContext tomlContext) {
        this.tomlContext = tomlContext;
        this.stack = new Stack(tomlContext.depthLimit());
        this.tokenizer = new TomlTokenizer(new InputStreamReader(in), tomlContext.bufferPool());
    }
    ...
    public TomlArray getArray() {
        ...
        return getArray(new TomlArrayBuilderImpl());
    }
...
//        switch (currentEvent) {
//            case START_ARRAY:
//                return getArray(new TomlArrayBuilderImpl(jsonContext));
//            case START_OBJECT:
//                return getObject(new TomlObjectBuilderImpl(jsonContext));
//            case KEY_NAME:
//            case VALUE_STRING:
//                return new TomlStringImpl(getCharSequence());
//            case VALUE_NUMBER:
//                if (isDefinitelyInt()) {
//                    return TomlNumberImpl.getJsonNumber(getInt(), jsonContext.bigIntegerScaleLimit());
//                } else if (isDefinitelyLong()) {
//                    return TomlNumberImpl.getJsonNumber(getLong(), jsonContext.bigIntegerScaleLimit());
//                }
//                return TomlNumberImpl.getJsonNumber(getBigDecimal(), jsonContext.bigIntegerScaleLimit());
//            case VALUE_TRUE:
...
//        throw parsingException(JsonToken.EOF, "[STRING, CURLYCLOSE]");
    ...
    private RuntimeException parsingException(TomlToken token, String expectedTokens) {
        ...
        return new RuntimeException(
            "Invalid token=%s at %s. Expected tokens are: %s".formatted(token, "" /*location*/, expectedTokens));
    }
