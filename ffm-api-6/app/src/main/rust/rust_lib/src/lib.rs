...
const FONT_DATA: &[u8] = include_bytes!("hw3.ttc");

#[unsafe(no_mangle)]
pub extern "C" fn vello_ctx_create(width: u32, height: u32) -> *mut RenderContext {
    let bytes_per_row = width * 4;
    assert_eq!(bytes_per_row % 256, 0, "Width * 4 must be a multiple of 256");
...
#[unsafe(no_mangle)]
pub extern "C" fn vello_ctx_render(ctx_ptr: *mut RenderContext, buffer: *mut u8, text_ptr: *const c_char) {
    if ctx_ptr.is_null() || buffer.is_null() || text_ptr.is_null()  { return; }
    ...
    for ch in text.chars() {
        if let Some(glyph_id) = char_map.map(ch) {
            let glyph = Glyph { id: glyph_id.to_u32(), x: current_x, y: 0.0, };
            glyphs.push(glyph);
    ...
    scene
        ...
        .transform(Affine::translate((50.0, 100.0)))
        .hint(true) // font hinting
